...
cat local.conf.master | sed "s/172.27.162.239/${IPADDR}/g" | sed "s/sf_account_prefix=balduf-liberty/sf_account_prefix=balduf-${SHORTNAME}/g" > /tmp/local.conf.master
cp /tmp/local.conf local.conf.master
rm devstack/local.conf
ln -s ../local.conf.master devstack/local.conf
